...
export class AppService {
  ...
  async getServiceHealth() {
    ...
    const [predictor, llm] = await Promise.all([
      axios
        .get(`${predictorUrl}/health`, { timeout: 3000 })
  ...
  private async getLlmHealth(options: {
    ...
    const { provider, openRouterKey, openRouterUrl, ollamaUrl, model } =
      options;

    if (provider === 'openrouter') {
      return await axios
        .get(`${openRouterUrl}/models`, {
          timeout: 5000,
          headers: {
    ...
    return await axios
      .get(`${ollamaUrl}/api/tags`, { timeout: 3000 })
      .then(() => ({ status: 'online' as const, provider, model }))
      .catch(() => ({ status: 'offline' as const, provider, model }));
  }
}
