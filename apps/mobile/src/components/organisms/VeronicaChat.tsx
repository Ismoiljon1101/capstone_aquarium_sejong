...
const WELCOME: Msg = {
  role: 'veronica',
  text: "Hi! I'm Veronica, your AI aquarium advisor. Tap 📞 to start a live voice call, or type below.",
  ts: new Date(),
};
...
export const VeronicaChat: React.FC<Props> = ({ onStateChange }) => {
  ...
  const startListeningCycle = useCallback(() => {
    setListening(true);
    const ok = sr.start(
      (transcript) => { setListening(false); askVeronica(transcript); },
      () => setListening(false),
    );
    if (!ok) {
      setListening(false);
      setCallActive(false);
      setMsgs(p => [...p, { role: 'veronica', text: 'Voice not supported here. Use Chrome/Edge.', ts: new Date() }]);
    }
  }, [sr, askVeronica]);
  ...
  const toggleCall = () => {
    if (callActive) {
      ...
      setCallActive(false);
      setListening(false);
      setMsgs(p => [...p, { role: 'veronica', text: 'Call ended. Tap 📞 anytime.', ts: new Date() }]);
    } else {
      setCallActive(true);
      setMsgs(p => [...p, { role: 'veronica', text: "Call started! I'm listening — speak now.", ts: new Date() }]);
      speak("Call started! I'm listening.");
      startListeningCycle();
    }
  };

  // Re-listen loop while call active
  useEffect(() => {
  ...
  return (
    <View style={{
      ...
      <View style={{
        ...
        <View style={{ width: 38, height: 38, borderRadius: 19, borderCurve: 'continuous', backgroundColor: '#0891b2', alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ fontSize: 18 }}>🧠</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 15, fontWeight: '800', color: '#f1f5f9' }}>Veronica AI</Text>
          <Text style={{ fontSize: 11, color: '#64748b', marginTop: 1 }}>
            {callActive ? (listening ? 'Listening...' : loading ? 'Thinking...' : 'Ready — speak') : 'AI Aquarium Advisor · Ollama'}
          </Text>
        </View>
        ...
        <TouchableOpacity onPress={toggleCall} activeOpacity={0.8} style={{ alignItems: 'center', justifyContent: 'center' }}>
          ...
          <View style={{
            ...
            borderWidth: 1.5, borderColor: callActive ? '#22c55e' : 'rgba(34,197,94,0.3)',
          }}>
        ...
        <View style={{
          ...
          borderWidth: 1, borderColor: callActive ? 'rgba(34,197,94,0.3)' : 'rgba(6,182,212,0.25)',
        }}>
          <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: callActive ? '#22c55e' : '#22d3ee' }} />
      ...
      {/* Live call banner */}
      {callActive && (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingVertical: 8, backgroundColor: 'rgba(34,197,94,0.08)', borderBottomWidth: 1, borderBottomColor: 'rgba(34,197,94,0.12)' }}>
      ...
      {/* Messages */}
      <ScrollView ref={scrollRef} style={{ maxHeight: 280 }} contentContainerStyle={{ padding: 14, gap: 10 }} onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: true })}>
        {msgs.map((m, i) => (
          <View key={i} style={{ flexDirection: 'row', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', gap: 8 }}>
            ...
            <View style={{
              ...
              borderWidth: 1, borderColor: m.role === 'user' ? 'rgba(29,78,216,0.5)' : 'rgba(255,255,255,0.07)',
            }}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={{ flexDirection: 'row', gap: 10, padding: 14, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.06)' }}>
          <TextInput
            style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.06)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', borderRadius: 14, borderCurve: 'continuous', paddingHorizontal: 14, paddingVertical: 10, fontSize: 14, color: '#e2e8f0' }}
            placeholder={callActive ? 'Or type while on call...' : 'Ask about pH, fish behavior...'}
            ...
            returnKeyType="send"
          />
          <TouchableOpacity onPress={send} disabled={!input.trim() || loading} activeOpacity={0.8} style={{ backgroundColor: input.trim() && !loading ? '#0891b2' : '#1e293b', borderRadius: 14, borderCurve: 'continuous', paddingHorizontal: 18, justifyContent: 'center', borderWidth: 1, borderColor: input.trim() && !loading ? '#0891b2' : 'rgba(255,255,255,0.06)' }}>
